...
export function buildContext(role: Role, accountId?: string): string {
  const lines: string[] = [];
  ...
  if (accountId) {
    ...
    if (account) {
      ...
      for (const d of getDealsForAccount(accountId)) {
  ...
  const cases = getAllCases().filter((c) => !accountId || c.accountId === accountId);
  for (const c of triageSort(cases)) {
    const sla = slaInfo(c);
    const req = requestStatus(c);
    lines.push(
      `CASE ${c.id} "${c.title}" [${getAccount(c.accountId)?.name}]: ${c.priority} priority, ${c.status}, ${caseAgeDays(c)}d old, ${sla.label}, ${req.label}${c.escalatedToThirdParty ? ", 3rd-party escalated" : ""}.`,
    );
    const internal = getNotesForCase(c.id).find((n) => n.visibility === "internal");
    if (internal) lines.push(`  internal note: ${internal.body}`);
  }

  if (accountId) {
    const hist = getServiceHistory(accountId).slice(0, 6);
    if (hist.length) {
...
export async function ask(
  ...
  const context = buildContext(role, accountId);
  ...
  return { text: deterministicAnswer(role, question, accountId), modelUsed: false };
}
...
function deterministicAnswer(role: Role, question: string, accountId?: string): string {
  ...
  if (/breach|sla|overdue|urgent|risk/.test(q)) {
    ...
    return (
      "SLA pressure:\n" +
      pressing.map((c) => `• ${c.title} (${getAccount(c.accountId)?.name}) — ${slaInfo(c).label}${c.escalatedToThirdParty ? ", 3rd-party" : ""}`).join("\n")
    );
  }
  ...
  const ctx = buildContext(role, accountId);
